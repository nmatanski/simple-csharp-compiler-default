...
namespace SimpleCSharpCompiler
{
	public class Emit
	{
		...
		public Emit(string name, Table symbolTable)
		{
			...
			var moduleName = Path.GetFileName(name);
		...
		{
			var param = method.DefineParameter(paramIndex, ParameterAttributes.None, paramName);
			return param;
		}
		...
		public void AddCast(Type castType, Type exprType)
		{
			if (!castType.IsValueType && !exprType.IsValueType)
				...
				else if (castType==typeof(SByte)) // && exprType==typeof(System.Int32))
		...
		{
			var result = il.DeclareLocal(localVarType);
		...
		private bool CanInitializeLocation(Type Ty) {
			if (!Ty.IsValueType) {
            	if (Ty == typeof(String)) {
                	...
                	var cons = Ty.GetConstructor(Type.EmptyTypes);
