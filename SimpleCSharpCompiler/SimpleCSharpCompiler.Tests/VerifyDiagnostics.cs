...
namespace SimpleCSharpCompiler.Tests
{
	...
	public class VerifyDiagnostics : Diagnostics
	{
		...
		private void GetExpectedErrors() {
			var reader = new StreamReader(currentSourceFile);
		...
		private void DumpSeenErrors() {
			...
			foreach (var error in SeenErrors) {
				Debug.WriteLine(error.ToString());
			}
			Debug.WriteLine("\n");
		}
		...
		private int CompareErrorLists() {
			GetExpectedErrors();
			for (var i = SeenErrors.Count - 1; i >= 0; --i) {
				var seenError = SeenErrors[i];
				// Check whether we had more than one error on line. In that case
				// we can neglect the exact ordering.
				for (var j = ExpectedErrors.Count - 1; j >= 0; --j) {
					var expectedError = ExpectedErrors[j];
