...
namespace SimpleCSharpCompiler.Tests
{
	[TestFixture]
	public class SimpleCSharpCompilerTestFixture
	{
		...
		{
			var testPath = TestCasesDir + testname;
			var testCasesTmpDir = TestCasesDir + "tmp" + Path.DirectorySeparatorChar;
			var sourceFilePath = testPath + ".scs";
			var exeFilePath =  testCasesTmpDir + testname+ ".exe";
			var resultFilePath = testPath + ".result";
			...
			var diag = new VerifyDiagnostics ();
			var Compiled = Compiler.Compile(sourceFilePath, exeFilePath, diag);
			...
			if (File.Exists(resultFilePath)) {
				var resultFileText = Normalize(File.ReadAllText(resultFilePath));
				var p = new Process();
				...
				var output = Normalize(p.StandardOutput.ReadToEnd());
				var error = Normalize(p.StandardError.ReadToEnd());
